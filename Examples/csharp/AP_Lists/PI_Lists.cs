...
namespace AP_Lists
{
	...
	[Guid("4FD0EBA8-79C7-4E54-AECB-2D4306337734"),
	ClassInterface(ClassInterfaceType.AutoDispatch),
	ComVisible(true)]
	public class PI_Lists : INxPlugInInterface, INxPlugInGuiEvents
	{
		#region Class Members
		const string c_PlugInName = "Lists PlugIn";
		const string c_CommandName_PlugInDesciption = "&Lists PlugIn...";
		const string c_CommandName_Lists = "&Lists...";
		...
		int INxPlugInInterface.Initialize(string PlugInId, NxProject Project, object GuiApiObj)
		{
			try
			{
				...
				if (_GuiApi != null)
				{
					...
					MenuUtils.AddMenuCommand(_GuiApi, MenuUtils.g_Local_MenuName_Tools, MenuUtils.g_Local_MenuName_Tools_SDK, c_CommandName_Lists, ref _CommandNumber_Lists);
				}
			}
			catch (Exception ex)
			{
		...
		void INxPlugInInterface.Uninitialize()
		{
			try
			{
				if (_GuiApi != null)
				{
					...
					MenuUtils.RemoveMenuCommand(_GuiApi, MenuUtils.g_Local_MenuName_Tools, MenuUtils.g_Local_MenuName_Tools_SDK, c_CommandName_Lists);
		...
		void INxPlugInGuiEvents.BeginCustomCommand(int CommandNumber, NxTbl WindowTbl, NxDetailedList DetailedList)
		{
			...
			try 
			{
				...
				{
					MessageBox.Show("Lists Administration");
				}
				...
				else if(CommandNumber == _CommandNumber_Lists)
				{
					ListsForm sf = new ListsForm();
