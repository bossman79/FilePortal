...
namespace AP_Search
{
	...
	[Guid("739F9189-E0CE-42EE-AC73-3812E588338F"),
	ClassInterface(ClassInterfaceType.AutoDispatch),
	ComVisible(true)]
	public class PI_Search : INxPlugInInterface, INxPlugInGuiEvents
	{
		#region Class Members
		const string c_PlugInName = "Search PlugIn";
		const string c_CommandName_PlugInDesciption = "&Search PlugIn...";
		const string c_CommandName_Search = "&Search...";
		...
		int INxPlugInInterface.Initialize(string PlugInId, NxProject Project, object GuiApiObj)
		{
			try
			{
				...
				if (_GuiApi != null)
				{
					...
					MenuUtils.AddMenuCommand(_GuiApi, MenuUtils.g_Local_MenuName_Tools, MenuUtils.g_Local_MenuName_Tools_SDK, c_CommandName_Search, ref _CommandNumber_Search);
				}
			}
			catch (Exception ex)
			{
		...
		void INxPlugInInterface.Uninitialize()
		{
			try
			{
				if (_GuiApi != null)
				{
					...
					MenuUtils.RemoveMenuCommand(_GuiApi, MenuUtils.g_Local_MenuName_Tools, MenuUtils.g_Local_MenuName_Tools_SDK, c_CommandName_Search);
		...
		void INxPlugInGuiEvents.BeginCustomCommand(int CommandNumber, NxTbl WindowTbl, NxDetailedList DetailedList)
		{
			...
			{	        
				...
				{
					MessageBox.Show("Search Administration");
				}
				...
				else if(CommandNumber == _CommandNumber_Search)
				{
					SearchForm sf = new SearchForm();
					sf._GuiApi = _GuiApi;
					sf._Project = _Project;
                    sf._WindowTbl = WindowTbl;
					sf.ShowDialog();
				}
			}
			catch (Exception ex)
